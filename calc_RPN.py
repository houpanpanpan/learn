...
def find_errors(string):#检查输入是否存在错误
        str1 = "0123456789+-*/()."
        flag = True
        m=0
        for i in string:
...
def process(string):#对负数进行处理，将-1变为0-1
    a=string
    b=0
...
class InversPolishCalculator(object):#逆波兰计算器
    ...
    def deal(self,string):#主程序,传入中缀表达式,返回结果
    ...
    def deal_str(self,string):#处理中缀表达式字符串,转为列表形式方便计算
        status = 0
        res = ''
    ...
    def get_list_expression(self,string):#将中缀表达式列表形式转后缀
